...
In una navbar potrebbe essere preferibile usare il componente `NavLink` che rispetto a `Link` ha il vantaggio di aggiungere la classe `active` se ci troviamo allo stesso indirizzo dell'attributo "to" dando la possibilità di renderizzare il link in maniera diversa.
