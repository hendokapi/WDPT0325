...
**_Una `statement` è un'istruzione che non ritorna un valore (ad esempio i costrutti `if`, `for`, `while`)._**
...
**_<u>QUESTO NON SI PUO' FARE!</u>_**
...
Alle variabile può essere assegnato un nome diverso da quello della chiave in questo modo:

```js
const { firstName: nome, lastName: cognome, age: anni } = objPerson;
```

equivale a scrivere:

```js
const nome = objPerson.firstName; // 'Pinco'
const cognome = objPerson.lastName; // 'Pallino'
const anni = objPerson.age; // 30
```
...
```js
const { firstName: name, lastName, age } = myObj;
```
